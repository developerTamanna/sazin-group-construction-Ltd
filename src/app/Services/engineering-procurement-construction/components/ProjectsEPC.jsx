...
const projects = [
  { title: 'Power Plant EPC Project', img: '/epc1.jpg' },
  { title: 'Industrial EPC Project', img: '/epc2.avif' },
  { title: 'Commercial EPC Turnkey', img: '/epc3.jpg' },
];

export default function ProjectsEPC() {
  return (
    <section className="bg-white dark:bg-neutral-950 py-16 px-6 lg:px-8">
      <h2 className="text-3xl lg:text-4xl font-bold text-center text-red-600">
        Sample EPC Projects
      </h2>
      ...
      <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {projects.map((p) => (
          <div
            key={p.title}
            className="group relative rounded-xl overflow-hidden border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-900 shadow-md hover:shadow-xl transition-shadow duration-300"
          >
            <Image
              src={p.img}
              alt={p.title}
